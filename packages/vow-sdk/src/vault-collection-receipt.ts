...
export async function readVaultCollectionReceipt(reader: PublicReader, deployment: VaultDeploymentManifest, transactionHash: bigint): Promise<VaultCollectionReceiptReport> {
  ...
  const report: VaultCollectionReceiptReport = { transactionHash, status: 'unknown', reason: 'VOW_RECEIPT_UNAVAILABLE', blockHash: null,
    finality: null, events: 'unverified', callPath: 'unverified', reservationId: null, mandateId: null, noteId: null, token: null,
    amount: null, signatureDeadline: null, noteOwnership: 'unverified', retryAllowed: false, source: 'single-rpc-observation' };
  try {
    ...
    const identified = { ...atBlock, reservationId: event.reservationId, mandateId: event.mandateId, noteId: event.noteId, amount: event.amount };
    ...
    const matched = { ...identified, token: reservation.token, events: 'matched' as const };
    let trace: unknown;
    ...
    await checkBlock();
    return { ...matched, status: 'confirmed', callPath: 'matched', signatureDeadline, reason: 'VOW_COLLECTION_CONFIRMED' };
  } catch (error: unknown) {
